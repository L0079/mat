...
function InvoiceRow2({ invoice }) {
  ...
  return (
    <Table.Row>
      ...
      <Item>{format(new Date(duePaymentDate), "dd MMM yyyy")}</Item>
      <Customer>{customer}</Customer>
      <Amount>{formatCurrency(amount, currency)}</Amount>
      <Amount>{formatCurrency(totalAmount, currency)}</Amount>
      <Item>{differenceInDays(new Date(), parseISO(duePaymentDate))}</Item>
      <Modal>
        <Menus.Menu>
...
InvoiceRow2.propTypes = { invoice: PropTypes.object };
export default InvoiceRow2;
